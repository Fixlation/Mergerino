...
{
    this->actionWidget->setEnabled(setting.getValue());
    setting.connect(
...
SettingWidget *SettingWidget::conditionallyEnabledBy(
    pajlada::Settings::Setting<QString> &setting, const QString &expectedValue)
{
    this->actionWidget->setEnabled(setting.getValue() == expectedValue);
    setting.connect(
