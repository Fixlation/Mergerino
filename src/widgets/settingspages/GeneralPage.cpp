...
void GeneralPage::initLayout(GeneralPageView &layout)
{
    ...
    SettingWidget::checkbox("Always on top", s.windowTopMost)
        ->setTooltip("Always keep Mergerino as the top window.")
        ->addTo(layout);

#ifdef USEWINSDK
    SettingWidget::checkbox("Start with Windows", s.autorun)
        ->setTooltip("Start Mergerino when your computer starts.")
        ->addTo(layout);
#endif
    ...
    layout.addDropdown<std::underlying_type_t<MessageOverflow>>(
        ...
        false,
        "Specify how Mergerino will handle messages that exceed Twitch "
        "message limits");
    layout.addDropdown<std::underlying_type_t<UsernameRightClickBehavior>>(
        ...
        false,
        "Specify how Mergerino will handle right-clicking a username in "
    ...
    SettingWidget::checkbox("Reduce opacity of message history",
        ...
        ->setTooltip(
            "Reduce opacity of messages that were posted before Mergerino "
            "was started or while reconnecting.")
        ->addTo(layout);
    ...
    SettingWidget::checkbox("Draw a line below the most recent message before "
        ...
        ->setTooltip("Adds an underline below the most recent message "
                     "sent before you tabbed out of Mergerino.")
        ->addTo(layout);
    ...
    SettingWidget::checkbox("Animate only when Mergerino is focused",
    ...
    SettingWidget::checkbox("Send activity to BetterTTV", s.sendBTTVActivity)
        ->setTooltip(
            "When enabled, Mergerino will signal an activity to BetterTTV "
    ...
    SettingWidget::checkbox("Send activity to 7TV", s.sendSevenTVActivity)
        ->setTooltip("When enabled, Mergerino will signal an activity to 7TV "
                     "when you send a chat message. This is used for badges, "
    ...
    layout.addDescription(
        "Mergerino can automatically change behavior if it detects that any "
...
#ifdef Q_OS_WIN
    layout.addDescription(
        "Mergerino exposes a native messaging host for compatible browser "
        "extensions. With a compatible extension, Twitch.tv chat can be "
        "replaced and the /watching split can stay in sync.");
#else
    layout.addDescription("Mergerino exposes a native messaging host for "
                          "compatible browser extensions that can update the "
                          "/watching split when Twitch.tv is open.");
#endif
    ...
    SettingWidget::checkbox("Attach to any browser (may cause issues)",
        ...
        ->setTooltip(
            "Attempt to force Mergerino browser integration to work in "
...

    {
        ...
        SettingWidget::lineEdit("Allowed extension IDs", s.additionalExtensionIDs,
    ...
    // Choose and reset buttons
    {
        ...
        box->addWidget(layout.makeButton("Clear Cache", [&layout]() {
            auto reply = QMessageBox::question(
                ...
                "take longer to load next time Mergerino is started.",
    ...
    SettingWidget::checkbox("Keep sound backend alive (requires restart)",
        ...
        ->setTooltip(
            "This setting makes Mergerino output silence to your sound "
    ...
    SettingWidget::customCheckbox(
        ...
        ->setTooltip("When possible, restart Mergerino if the program crashes")
        ->addTo(layout);
    ...
    SettingWidget::checkbox("Color @usernames", s.colorUsernames)
        ->setTooltip("If Mergerino has seen a user, highlight @mention's of "
