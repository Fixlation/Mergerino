    ...
    , notebook_(new SplitNotebook(this))
{
    ...
    this->signalHolder_.managedConnect(
        ...
        [this](ProviderId) { this->onAccountSelected(); });
    this->onAccountSelected();
...
void Window::addMenuBar()
{
    ...
    // About button opens the settings dialog.
    ...
    auto ensureHelpMenu = [&]() -> QMenu * {
        if (helpMenu == nullptr)
        {
            helpMenu = mainMenu->addMenu(QString("Help"));
        }
        return helpMenu;
    };

    if (!LINK_MERGERINO_DOCUMENTATION.isEmpty())
    {
        QAction *helpDocs = ensureHelpMenu()->addAction(QString("Documentation"));
        connect(helpDocs, &QAction::triggered, this, []() {
            QDesktopServices::openUrl(
                QUrl(LINK_MERGERINO_DOCUMENTATION.toString()));
        });
    }

    if (!LINK_MERGERINO_SOURCE.isEmpty())
    {
        QAction *helpSource = ensureHelpMenu()->addAction(QString("Source Code"));
        connect(helpSource, &QAction::triggered, this, []() {
            QDesktopServices::openUrl(QUrl(LINK_MERGERINO_SOURCE.toString()));
        });
    }

    if (!LINK_MERGERINO_DISCORD.isEmpty())
    {
        QAction *helpDiscord = ensureHelpMenu()->addAction(QString("Discord"));
        connect(helpDiscord, &QAction::triggered, this, []() {
            QDesktopServices::openUrl(QUrl(LINK_MERGERINO_DISCORD.toString()));
        });
    }
}
...
void Window::onAccountSelected()
{
    ...
    const auto accountLabel = providerAccountLabel(provider);

    // update title (also append username on Linux and MacOS)
    QString windowTitle = Version::instance().fullVersion();

#if defined(Q_OS_LINUX) || defined(Q_OS_MACOS)
    windowTitle += QString(" - %1: %2").arg(providerName(provider),
                                            accountLabel.toLower() == "log in"
                                                ? "not logged in"
                                                : accountLabel);
#endif
